...
const DeepMatrixVisualization: React.FC<DeepMatrixVisualizationProps> = ({
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(
      40, // Narrower field of view to zoom out
    ...
    const pointsMaterial = new THREE.PointsMaterial({
      ...
      size: 1, // Smaller size for proper "pixel" appearance
    });
    ...
    allTokenPositions.forEach((stack) =>
      stack.forEach((layer) => {
        ...
        parentGroup.add(points); // Add points to the rotating parent group
      })
    );
    ...
    for (const stack of allTokenPositions) {
      for (let layerIndex = 0; layerIndex < stack.length; layerIndex++) {
        ...
        // Intra-layer connections (within the same layer)
        currentLayer.forEach((start, t) => {
          ...
          for (let i = 0; i < maxConnectionsPerToken; i++) {
            ...
            const mat = new THREE.LineBasicMaterial({
              color: 0x808080, // Gray for inactive
              transparent: true,
              opacity: 0.2,
            });
